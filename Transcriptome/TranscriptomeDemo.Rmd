...
# Table for the 4 observed data sets from the planktonic and biofilm sets
```{r}
...
# Given the type of gene (if provided) and features
```{r}
results_transcripts[results_transcripts$transcriptNames == "gene-PA0280", ]
```

# Significant differences in the transcript results
```{r}
sigdiff <- results_transcripts %>% filter(pval<0.05)
dim(sigdiff)
```

# organize the table <organized by increasing pvalue>
```{r}
...
## The amount of genes with increasing expression through the amount of transcripts in each.

# create a plot of how similar the two replicates are for one another. We have two data sets...replacing out plank01 and plank02 with biofilm01 and biofilm02 to plot the other set.
```{r}
...
## If the two data sets are similar then the two sets of genes have similar gene expression.
...
## interpret the above figure: the expression of the planktonic genes compared to the biofilm genes based on their folding change.
...
```{r}
ballgown::transcriptNames(bg_filt)[12]
ballgown::geneNames(bg_filt)[12]
```
# transform to log2
```{r}
...
## planktonic has an increased value in expression based on fragments per kilobase of transcript per million mapped reads than biofilm on gene-PA0002.